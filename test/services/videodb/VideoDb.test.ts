...
describe('VideoDb', () => {
    ...
    describe('queryVideos', () => {
        ...
        it('runs the correct sql with filter params when titleContains, categories, tags, watchedStatuses, pmWatchedStatuses and maxLength filter param are all defined', async () => {
            mockStorage.contentFileExists.mockReturnValue(true);
            mockGet.mockResolvedValueOnce({ ver: 4 });
            await videoDb.initialise();
            const expectedSql = baseSql + ` WHERE (length_mins <= $maxLength)
                                            ...
                                            AND (primary_media_watched IN ($pmWatchedStatus0, $pmWatchedStatus1, $pmWatchedStatus2))` + orderBySQL;
            const expectedParams = {
                '$titleContains': '%title%',
