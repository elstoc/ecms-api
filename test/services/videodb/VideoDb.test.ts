...
describe('VideoDb', () => {
    ...
    describe('queryVideos', () => {
        ...
        it('runs the correct sql with filter params when titleContains, categories, tags, watchedStatuses and maxLength filter param are all defined', async () => {
            mockStorage.contentFileExists.mockReturnValue(true);
            mockGet.mockResolvedValueOnce({ ver: 4 });
            await videoDb.initialise();
            const expectedSql = baseSql + ` WHERE (length_mins <= $maxLength)
                                            ...
                                            AND (watched IN ($watchedStatus0, $watchedStatus1, $watchedStatus2))` + orderBySQL;
            const expectedParams = {
                '$titleContains': '%title%',
            ...
            mockGetAllWithParams.mockResolvedValue('videos');
            const videos = await videoDb.queryVideos({ titleContains: 'title', maxLength: 120, categories: ['MOV','TV','TVDOC'], tags: ['tag0','tag1','tag2'], watchedStatuses: ['Y', 'N', 'P'] });
